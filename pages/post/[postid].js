...
class article extends Component {
	...
	render() {
		...
		return (
			<div>
				<Container>
					<Card key={article.id} style={{ backgroundColor: 'transparent' }}>
						<Card.Img variant="top" src={article.featuredImage} />
						<Media>{article.video}</Media>
						<Card.Body>
							<Card.Title>
								<h1 style={{ fontFamily: 'DINCondensed-Bold' }}>{article.title}</h1>{' '}
							</Card.Title>
							<br />
							<Card.Subtitle style={{ fontFamily: 'ina01-Regular' }}>
								{' '}
								Authored by {article.author} | Published{' '}
